export async function onRequest(context) {
  ...
  try {
    ...
    if (post.creator_steamid !== steamid) {
      return new Response(JSON.stringify({ error: 'Only creator can close the post' }), { status: 403, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
    }
    if (post.status === 0) {
      return new Response(JSON.stringify({ error: 'Post already closed' }), { status: 400, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
    }
