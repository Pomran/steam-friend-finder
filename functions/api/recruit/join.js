export async function onRequest(context) {
  ...
  try {
    ...
    if (post.status !== 1) {
      return new Response(JSON.stringify({ error: 'Post is closed' }), { status: 400, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
    }
    const members = safeJsonParse(post.member_list);
    if (members.length >= post.max_members) {
      return new Response(JSON.stringify({ error: 'Team is full' }), { status: 400, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
    }
    if (members.some(m => m.steamid === steamid)) {
      return new Response(JSON.stringify({ error: 'Already a member' }), { status: 400, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
    }
